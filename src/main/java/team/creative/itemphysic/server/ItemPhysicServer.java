...
public class ItemPhysicServer {
    ...
    public static void updateBurn(ItemEntity item) {
        try {
            ...
            if (item.world.getFluidState(item.getPosition()).isTagged(FluidTags.LAVA) && ItemPhysic.CONFIG.general.burningItems.canPass(item.getItem())) {
                ...
                item.remove();
                for (int i = 0; i < 100; i++)
                    item.world.addParticle(ParticleTypes.SMOKE, item.getPosX(), item.getPosY(), item
                            .getPosZ(), (rand.nextFloat() * 0.1) - 0.05, 0.2 * rand.nextDouble(), (rand.nextFloat() * 0.1) - 0.05);
            }
            ...
            if (ItemPhysic.CONFIG.general.enableIgniting && !item.world.isRemote && item.isOnGround() && Math.random() <= 0.1 && ItemPhysic.CONFIG.general.ignitingItems
    ...
    public static void onCollideWithPlayer(ItemEntity entity, PlayerEntity player, boolean needsSneak) {
        ...
        if (!entity.world.isRemote) {
            ...
            try {
                if ((!entity.cannotPickup() || ItemPhysic.CONFIG.pickup.customPickup) && (entity.getOwnerId() == null || entity.lifespan - age.getInt(entity) <= 200 || entity
                        .getOwnerId().equals(player.getUniqueID())) && (hook == 1 || i <= 0 || player.inventory.addItemStackToInventory(itemstack))) {
